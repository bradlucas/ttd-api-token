...
The `home-routes` handler in the `ttd-api-token.routes.home` namespace
...
The `home-page` function will in turn call the `ttd-api-token.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `ttd-api-token.handler` namespace:
...
Request middleware functions are located under the `ttd-api-token.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ttd-api-token.dev-middleware` namespace found in
